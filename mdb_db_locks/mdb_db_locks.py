    ...
    def metricCollector(self):
        ...
        try:
            ...
            try:
                ...
                locks_data = db.command("currentOp")
                if locks_data:
                    i=1
                    for lock_data in locks_data['inprog']:
                        if lock_data['waitingForLock']:
                            data['lock_opid_'+str(i)]=lock_data['opid']
                            i+=1
                        if i>10:
                            break
                    if i<10:
                        k=i
                        for j in range(11-i):
                            data['lock_opid_'+str(k)]=0
                            k+=1
                else:
                    for i in range(10):
                        data['lock_opid_'+str(i+1)]=0
